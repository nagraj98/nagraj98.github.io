...
const ChessPuzzle: React.FC = () => {
  ...
  const showSolution = () => {
    const tempGame = new Chess(game.fen()); // Create a temporary game instance to simulate moves without affecting the actual game state
    const solutionMove = tempGame.moves({ verbose: true }).find((move) => {
      tempGame.move(move);
      const isCheckmate = tempGame.isCheckmate();
      tempGame.undo(); // Undo the move to restore the state
      return isCheckmate;
    });
  ...
  // Helper function to translate piece codes to full names
